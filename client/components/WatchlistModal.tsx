...
export function WatchlistModal({ isOpen, onClose }: WatchlistModalProps) {
  ...
  const removeFromWatchlist = (id: number) => {
    ...
    localStorage.setItem('moviestream-watchlist', JSON.stringify(updatedWatchlist));
  };
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden">
        <DialogHeader className="border-b border-border pb-4">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-2xl font-bold">My Watchlist</DialogTitle>
          ...
          {watchlist.length === 0 ? (
            ...
            <div className="space-y-4">
              {watchlist.map((item) => {
                ...
                const title = isMovie ? (item as WatchlistItem).title : (item as WatchlistTVItem).name;
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center space-x-3 text-sm text-muted-foreground mb-2">
                        ...
                        <span className="genre-tag">{item.type === 'movie' ? 'Movie' : 'TV Show'}</span>
                      </div>
...
export const addToWatchlist = (item: TMDBMovie | TMDBTVShow, type: 'movie' | 'tv') => {
  const watchlistItem = {
    ...item,
    type,
  ...
  const exists = watchlist.some(existingItem => existingItem.id === item.id && existingItem.type === type);
  if (exists) {
